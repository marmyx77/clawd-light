...
/// A real clawd-light instance, started against a fake home.
///
...
final class AppUnderTest {
    ...
    init(binaryURL: URL, port: UInt16, home: URL? = nil) {
        ...
        self.home = home ?? FileManager.default.temporaryDirectory
            .appendingPathComponent("clawd-light-e2e-\(port)-\(ProcessInfo.processInfo.processIdentifier)")
    }
    ...
    func start() throws {
        ...
        for sub in [".claude/ide", ".claude/sessions", ".clawd-light"] {
    ...
    var tokenFileURL: URL {
        home.appendingPathComponent(".clawd-light/token")
    }
    ...
    func runHookScript(
        ...
        let script = home.appendingPathComponent(".clawd-light/hook.sh")
        let process = Process()
